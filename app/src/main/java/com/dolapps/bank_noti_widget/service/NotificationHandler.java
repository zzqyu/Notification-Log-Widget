	...
	private void updateBalance(String tableName,  String content) {
		try {
			if(content != null) {
				...
				String[] info = Util.getAccountBalance(packageName, text);
				if(info!=null) {
					synchronized (LOCK) {
				...
				else Toast.makeText(context, Util.getAppNameFromPackage(context, packageName, false)+"은(는) 은행알림위젯에서 지원하지 않는 앱 입니다. ", Toast.LENGTH_SHORT).show();
			}
		} catch (Exception e) {
